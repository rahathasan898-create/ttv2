...
async function compareDependencyVersions(autoUpdatesImports, workDir, fetchFn = globalThis.fetch) {
  ...
  for (const [pkg, value] of filteredAutoUpdatesImports) {
    const resolvedVersion = await getRemoteResolvedVersion(fetchFn, value), dependency = dependencies[pkg], manifestPath = resolveFrom__default.default.silent(workDir, path__default.default.join(pkg, "package.json")), installed = semver__default.default.coerce(manifestPath ? (await readPackageManifest(manifestPath)).version : dependency);
    if (!installed)
