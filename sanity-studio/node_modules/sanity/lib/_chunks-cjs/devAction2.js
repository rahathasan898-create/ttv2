...
async function startSanityDevServer(args, context) {
  ...
  const autoUpdatesEnabled = shouldAutoUpdate.shouldAutoUpdate({
    flags,
    cliConfig
  }), cleanSanityVersion = semver__default.default.parse(installedSanityVersion)?.version;
  if (autoUpdatesEnabled && !cleanSanityVersion)
    throw new Error(`Failed to parse installed Sanity version: ${installedSanityVersion}`);
  const version = encodeURIComponent(`^${cleanSanityVersion}`), autoUpdatesImports = shouldAutoUpdate.getStudioAutoUpdateImportMap(version);
  if (autoUpdatesEnabled) {
    output.print(`${logSymbols.info} Running with auto-updates enabled`);
    let result;
    try {
      result = await shouldAutoUpdate.compareDependencyVersions(autoUpdatesImports, workDir);
    } catch (err) {
